...
WT="$FB_DIR/upstream-wt"
GITUP=(git -C "$UPSTREAM_PATH")
GREMOTE="$("${GITUP[@]}" remote -v | awk -v repo="$UPSTREAM_REPO" \
  '$0 ~ repo && $3 == "(fetch)" {print $1; exit}')"
[ -n "$GREMOTE" ] || die "上游 clone 无指向 $UPSTREAM_REPO 的远程"
"${GITUP[@]}" fetch "$GREMOTE" main --quiet
BASE="$("${GITUP[@]}" rev-parse --verify "$GREMOTE/main^{commit}")" \
  || die "无法解析 $GREMOTE/main"
...
GITW=(git -C "$WT")
say "上游 worktree: $WT 分支: $BRANCH (基点 $GREMOTE/main@${BASE:0:9})"
...
# --- 6. 确定性验证（不信任节点自觉）：退出码 / 产物 / 提交数 / 周界 / 干净树 ---
if [ "$NODE_RC" != 0 ] || [ -z "$ART_PATH" ] || [ ! -f "$ART_PATH" ]; then
  abandon
  die "适配节点失败或未产出 ARTIFACT（日志: $FB_DIR/adapt.log）"
fi
...
# --- 7. 上游门禁：红 → 不开 PR，只收报告 ---
say "==> 上游门禁: scripts/run_tests.sh --no-lock"
if (cd "$WT" && bash scripts/run_tests.sh --no-lock) \
...
PR_URL="$(gh pr create --repo "$UPSTREAM_REPO" --head "$BRANCH" \
  ...
  --body-file "$PR_BODY")" || die "gh pr create 失败（分支已推送: $GREMOTE/$BRANCH）"
say "✓ 上游 PR: $PR_URL"
