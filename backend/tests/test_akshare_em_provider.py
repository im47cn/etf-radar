...
@patch('src.providers.akshare_em_provider.ak.fund_etf_hist_em')
...
@patch('src.providers.akshare_em_provider.time.sleep')
@patch('src.providers.akshare_em_provider.ak.fund_etf_hist_em')
def test_retries_then_succeeds(mock_hist: MagicMock, mock_sleep: MagicMock) -> None:
    ...
    p = AkshareEmProvider(max_retries=3, base_delay=2.0)
    df = p.fetch_ohlc('512480', 5)
    assert not df.empty
...
def test_all_retries_exhausted_raises_provider_error(
    ...
    p = AkshareEmProvider(max_retries=3, base_delay=0.01)
...
def test_empty_data_not_retried(mock_hist: MagicMock, mock_sleep: MagicMock) -> None:
    mock_hist.return_value = pd.DataFrame()
    p = AkshareEmProvider(max_retries=3)
    with pytest.raises(EmptyDataError):
        p.fetch_ohlc('999999', 5)
