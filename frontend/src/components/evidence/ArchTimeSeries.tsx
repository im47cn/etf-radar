import {
  Bar, CartesianGrid, ComposedChart, Legend, Line, ReferenceLine, ResponsiveContainer, Tooltip, XAxis, YAxis,
} from 'recharts';
import { ChartCard, EmptyCard } from '@/components/ChartCard';

interface TimePoint {
  period: string;
  arch_ratio: number | null;
  arch_count: number | null;
  tested: number | null;
}

interface Props {
  timeSeries: TimePoint[];
}

/** ARCH 显著比例滚动时序 (120 日窗口按月步进) + MA3 趋势线. n≈120 功效充足, 给出可比的真实强度时序. */
export const ArchTimeSeries = ({ timeSeries }: Props) => {
  // for 循环 + 独立 const (非 map 返回多行对象字面量), 避免 v8 coverage 对象体盲区
  const ratios: number[] = [];
  for (const e of timeSeries) {
    ratios.push(e.arch_ratio ?? 0);
  }
  const data: { period: string; ratio: number; ma3: number }[] = [];
  for (let i = 0; i < timeSeries.length; i++) {
    const start = Math.max(0, i - 2);          // MA3 = 当前及前 2 月均值
    let sum = 0;
    for (let k = start; k <= i; k++) {
      sum += ratios[k];
    }
    const ma3 = sum / (i - start + 1);
    data.push({ period: timeSeries[i].period, ratio: ratios[i] * 100, ma3: ma3 * 100 });
  }
  if (data.length === 0) {
    return <EmptyCard text="暂无 ARCH 时序数据" />;
  }

  return (
    <ChartCard
      title="ARCH 显著比例（逐月·120日滚动）"
      subtitle="波动率聚集强度时序 · 柱=月度 · 线=MA3 趋势"
      helpTitle="ARCH 显著比例时序 · 读法"
      help={
        <>
          <p>柱 = 该月末向前 120 日窗口内 r² Ljung-Box p&lt;0.05 的主题占比（ARCH 显著 = 波动率聚集明显）。</p>
          <p><strong>为何用 120 日滚动</strong>：日历季仅 ~40–66 日、严重欠功率，单季比例被压到 0–14%，绝对值无意义。120 日窗口功效充足，实测范围 3–66%、均值 ~22%，与全样本 87% 可比，反映强度随时间的真实变化。</p>
          <p><strong>MA3 趋势线</strong>：3 月移动平均，平滑月度波动、凸显趋势方向。因 120 日窗口相邻已重叠 ~82%（本身极平滑），MA3 仅作轻量辅助——看拐点方向，勿当独立信号。</p>
          <p><strong>解读</strong>：峰值（如 2024-10 ≈66%）= 该窗口多数主题出现波动聚集，常对应行情剧烈阶段；低谷（&lt;10%）= 普遍平静。</p>
          <p><strong>用途</strong>：波动环境描述符——判断当前处于波动聚集期还是平静期，辅助仓位规模与风控。</p>
          <p><strong>⚠ 避免误用</strong>：ARCH 是对称度量，<strong>无方向预测力</strong>——实测 ARCH 显著比例 vs 各大指数月度涨跌 r≈0、p&gt;0.7。高 ARCH ≠ 要涨或要跌，只代表波动可能延续。勿用于方向择时，要判方向看 IC 滚动图。</p>
        </>
      }
    >
      <ResponsiveContainer width="100%" height={200}>
        <ComposedChart data={data} margin={{ top: 16, right: 12, bottom: 8, left: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
          <XAxis dataKey="period" tick={{ fontSize: 9 }} interval={5} />
          <YAxis tick={{ fontSize: 10 }} width={40} unit="%" domain={[0, 100]} />
          <Tooltip />
          <Legend wrapperStyle={{ fontSize: 11 }} />
          <ReferenceLine y={50} stroke="#cbd5e1" strokeDasharray="3 3" />
          <Bar dataKey="ratio" fill="#7c3aed" radius={[2, 2, 0, 0]} name="月度占比" />
          <Line dataKey="ma3" stroke="#ef4444" dot={false} strokeWidth={2} name="MA3 趋势" />
        </ComposedChart>
      </ResponsiveContainer>
    </ChartCard>
  );
};
