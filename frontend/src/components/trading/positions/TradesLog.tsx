...
/**
 * 交易事件流（最新在前）。删除/编辑均用于录错修正：
 * 删除两步行内确认（「删除」→「确认删除」）；编辑展开 TradeEditForm（删旧插新）。
 * 清仓事件超 7 天锁定（删除使已平仓交易复活，且修改会篡改已复盘事实）。
 */
        ...
        {rows.map((t) => {
          ...
          return (
            <li key={t.id} className="flex flex-wrap items-center gap-2 py-1.5 text-xs text-gray-700">
                ...
                <span className="ml-auto">
                  {isLocked ? (
                    <span
                      ...
                      title="清仓事件超过 7 天不可删除或修改：删除会使已平仓交易在复盘中复活，修改会篡改已复盘事实（历史事实保护）"
                    >
                      已锁定
                    </span>
                  ) : (
                    <>
                      <button
                        ...
                        onClick={() => { setEditId(t.id); setConfirmId(null); setError(null); }}
                      >
                        编辑
                      </button>
                      <button type="button" className={`ml-1 ${btnClass}`} disabled={isBusy} onClick={() => onRemove(t.id)}>
                        {isBusy ? '删除中...' : btnLabel}
                      </button>
                    </>
                  )}
              ...
              {isEditing && <TradeEditForm trade={t} onDone={() => setEditId(null)} />}
            </li>
          );
        })}
      </ul>
      <div className="mt-1 text-[10px] text-gray-400">删除/编辑仅用于录错修正：修改一笔事件后，持仓将按剩余事件流重新推导。清仓事件超过 7 天自动锁定（删除会使已平仓交易复活）。</div>
      {error && (
